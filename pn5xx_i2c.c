                           ...
                           struct pn544_i2c_platform_data *pdata)
{
    ...
    val = of_get_named_gpio_flags(node, "nxp,pn54x-ven", 0, &flags);
	if (val >= 0) {
		pdata->ven_gpio = val;
	}
    else{
	...
	val = of_get_named_gpio_flags(node, "nxp,pn54x-firm", 0, &flags);
    if (val >= 0) {
		pdata->firm_gpio = val;
    }
    else {
	...
	val = of_get_named_gpio_flags(node, "nxp,pn54x-irq", 0, &flags);
	if (val >= 0) {
		pdata->irq_gpio = val;
	}
    else {
